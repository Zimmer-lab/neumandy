...
def lorenz(t, y, controls=None, s=10, r=25, b=2.667):
    """
    Inputs:
    ...
    if controls is not None:
        x_dot += controls['uf'](t)
        y_dot += controls['uf2'](t)
        z_dot += controls['uf3'](t)
    return x_dot, y_dot, z_dot


def create_lorenz_mat(t=[], initial_conds=(0., 1., 1.05), txy=[], with_control=False):
    """
    ...
    if with_control:
        ...
        u2 = np.random.randn(t.shape[0])  # random noise
        uf2 = interp1d(t, u2, kind='previous')

        u3 = 10*np.sin(t * 0.2)
        uf3 = interp1d(t, u3, kind='previous')

        controls = {'uf': uf, 'uf2': uf2, 'uf3': uf3}

    fun = partial(lorenz, controls=controls)
    rtol = 10e-12
    atol = 10e-12 * np.ones_like(initial_conds)
    x = odeint(fun, initial_conds, t[:-1], tfirst=True, rtol=rtol, atol=atol)
    return x[:, 0], x[:, 1], x[:, 2]


def normalize_matrix(A):
    eigvals, eigvecs = np.linalg.eig(A)
    max_eigval = max(np.abs(eigvals))
    if max_eigval > 1:
        A = A / (max_eigval + 0.1)  # Damping factor to ensure stability
    return A
...
def create_dynamics(type_dyn='cyl', max_time=1000, dt=0.01,
                    params_ex={'radius': 1, 'num_cyls': 5, 'bias': 0, 'exp_power': 0.2}, with_control=False):
    """
    ...
    if type_dyn == 'cyl':
        ...
        x, y, z = create_lorenz_mat(t, txy=txy, with_control=with_control)
        dynamics = np.vstack([x.flatten(), y.flatten(), z.flatten()])
    elif type_dyn == 'torus':
        R = 5
        r = 1
        ...
        for dyn_num in range(params_ex['num_cyls']):
            ...
            random_trans = np.random.rand(
                dynamics0.shape[0], dynamics0.shape[0])-0.5
        ...
        mat_c_elegans = load_mat_file(
            'WT_NoStim.mat', 'E:\CoDyS-Python-rep-\other_models')
...
def train_model_include_D(max_time=500, dt=0.1, dynamics_type='cyl', num_subdyns=3,
    ...
    """
    # if D != I
    if not include_D and len(data) > 1:
        latent_dyn = data
    ...
    if len(data) == 0:
        data = create_dynamics(type_dyn=dynamics_type,
                               max_time=max_time, dt=dt, params_ex=params_ex)
        if not include_D:
            latent_dyn = data
    else:
    ...
    if include_D:  # If model needs to study D
        if len(D) == 0:
            D = init_mat(
                size_mat=(data.shape[0], latent_dim), dist_type='sparse', init_params={'k': 4})
        elif D.shape[0] != data.shape[0]:
            raise ValueError('# of rows in D should be = # rows in the data ')

    else:
        latent_dyn = data

    if len(F) == 0:
        F = [init_mat((latent_dim, latent_dim), normalize=True,
  ...
  """
    ...
    if len(params) == 0:
        params = {'update_c_type': 'inv', 'reg_term': 0, 'smooth_term': 0}

    if not include_D:
        cur_reco = create_reco(latent_dyn=latent_dyn,
                               coefficients=coefficients, F=F)
    ...
    """
        ...
        F = update_f_all(latent_dyn, F, coefficients, step_f, normalize=False,
        ...
        if np.mean(np.abs(np.diff(error_reco_array[-5:]))) < epsilon_error_change:
            F = [f_i + sigma_mix_f *
                 np.random.randn(f_i.shape[0], f_i.shape[1]) for f_i in F]
            print('mixed F')

        if to_print:
            print('Error = %s' % str(error_reco))
            if include_D:
                print('Error reco y = %s' % str(data_reco_error))
...
def update_D(former_D, step_D, x, y, reg1=0, reg_f=0):
    ...
    if reg1 == 0 and reg_f == 0:
        ...
        if reg1 != 0:
            reg1_error = np.sum(np.sign(former_D))
        else:
            reg1_error = 0
        if reg_f != 0:
            reg_f_error = 2*former_D
        reg_f_error = 0
        D = former_D - step_D * (basic_error + reg1 *
                                 reg1_error + reg_f * reg_f_error)
    return D
...
def update_X(D, data, reg1=0, former_x=[], random_state=0, other_params={}):
    ...
    if reg1 == 0:
        ...
        clf = linear_model.Lasso(
            alpha=reg1, random_state=random_state, **other_params)
        clf.fit(D, data)
...
def check_F_dist_init(F, max_corr=0.1):
    ...
    corr_bool = [spec_corr(F[comb_s[0]], F[comb_s[1]]) >
                 max_corr for comb_s in combs]
    counter = 100
    while (corr_bool == False).any():
        ...
        for comb_num, comb in enumerate(combs):
            if spec_corr(F[comb[0]], F[comb[1]]) > max_corr:
                fi_new = init_mat(
                    np.shape(F[0]), dist_type='norm', r_seed=counter)
                F[comb[0]] = fi_new
    return F
...
def init_mat(size_mat, r_seed=0, dist_type='norm', init_params={'loc': 0, 'scale': 1}, normalize=False):
    ...
    if dist_type == 'norm':
        ...
        if 'high' not in init_params.keys() or 'low' not in init_params.keys():
            raise KeyError(
                'Initialization did not work since low or high boundries were not set')
        rand_mat = np.random.uniform(
            init_params['low'], init_params['high'], size=size_mat)
    elif dist_type == 'inti':
        if 'high' not in init_params.keys() or 'low' not in init_params.keys():
            raise KeyError(
                'Initialization did not work since low or high boundries were not set')
        ...
        k = init_params['k']
        b1 = [random.sample(list(np.arange(size_mat[0])), np.random.randint(
            1, np.min([size_mat[0], k]))) for i in range(size_mat[1])]
        ...
        if 'k' not in init_params.keys():
            raise KeyError(
                'Initialization did not work since k was not set for regional initialization')

        k = init_params['k']
        splits = [len(split) for split in np.split(np.arange(size_mat[1]), k)]
        cur_repeats = [np.repeat(np.eye(size_mat[0]), int(
            np.ceil(split_len/size_mat[0])), axis=1) for split_len in splits]
...
def update_c(F, latent_dyn,
    ...
    for time_point in np.arange(n_times):
        ...
        if params_update_c['update_c_type'] == 'inv' or (params_update_c['reg_term'] == 0 and params_update_c['smooth_term'] == 0):
            ...
            coeffs = pylops.optimization.sparsity.FISTA(Aop, total_next_dyn_full.flatten(
            ), niter=params_update_c['num_iters'], eps=params_update_c['reg_term'], threshkind=params_update_c.get('threshkind'))[0]
            ...
            # print('omp')
            Aop = pylops.MatrixMult(stacked_fx_full)
            coeffs = pylops.optimization.sparsity.OMP(Aop, total_next_dyn_full.flatten(
            ), niter_outer=params_update_c['num_iters'], sigma=params_update_c['reg_term'])[0]
            ...
            coeffs = pylops.optimization.sparsity.IRLS(
                Aop, total_next_dyn_full.flatten(),  nouter=50, espI=params_update_c['reg_term'])[0]
...
def create_next(latent_dyn, coefficients, F, time_point):
    ...
    if latent_dyn.shape[1] > 1:
        ...
        cur_A = np.dstack([coefficients[i, time_point]*f_i @
                          latent_dyn for i, f_i in enumerate(F)]).sum(2).T
    return cur_A
...
def create_ci_fi_xt(latent_dyn, F, coefficients, cumulative=False, mute_infs=10**50,
    ...
    for time_point in np.arange(latent_dyn.shape[1]-1):
        if cumulative:
            ...
            cur_A = create_next(np.reshape(
                previous_A, [-1, 1]), coefficients, F, time_point)
        else:
...
def update_f_all(latent_dyn, F, coefficients, step_f, normalize=False, acumulated_error=False,
    ...
    if action_along_time == 'mean':
        ...
        new_f_s = [norm_mat(f_i-2*step_f*norm_mat(np.mean(all_grads[:, :, :]*np.reshape(coefficients[i, :], [
                            1, 1, -1]), 2), to_norm=normalize), to_norm=normalize_eig) for i, f_i in enumerate(F)]
    elif action_along_time == 'median':
        all_grads = create_ci_fi_xt(latent_dyn, F, coefficients)

        new_f_s = [norm_mat(f_i-2*step_f*norm_mat(np.median(all_grads[:, :, :]*np.reshape(coefficients[i, :], [
                            1, 1, -1]), 2), to_norm=normalize), to_norm=normalize_eig) for i, f_i in enumerate(F)]
...
def add_bar_dynamics(coefficients_n, ax_all_all=[], min_max_points=[10, 100, 200, 300, 400, 500],
    ...
    if isinstance(ax_all_all, list) and len(ax_all_all) == 0:
        fig, ax_all_all = plt.subplots(
            1, len(min_max_points), figsize=(8*len(min_max_points), 7))
...
def plot_sub_effect(sub_dyn, rec_rad_all=5, colors=['r', 'g', 'b', 'm'], alpha=0.8, ax=[],
    ...
    if len(colors) == 1:
        colors = [colors]*4
    if not isinstance(rec_rad_all, list):
        rec_rad_all = [rec_rad_all]
    ax.axhline(0, alpha=0.1, color='black', ls='dotted')
    ax.axvline(0, alpha=0.1, color='black', ls='dotted')
    for rec_rad in rec_rad_all:
        ax.plot([-rec_rad, rec_rad], [rec_rad, rec_rad],
                alpha=alpha**2, color=colors[0], ls='--', lw=lw)
        ax.plot([-rec_rad, rec_rad], [-rec_rad, -rec_rad],
                alpha=alpha**2, color=colors[1], ls='--', lw=lw)
        ax.plot([rec_rad,  rec_rad], [-rec_rad, rec_rad],
                alpha=alpha**2, color=colors[2], ls='--', lw=lw)
        ax.plot([-rec_rad, -rec_rad], [-rec_rad,  rec_rad],
                alpha=alpha**2, color=colors[3], ls='--', lw=lw)

        if not (sub_dyn == 0).all():
            sub_dyn = norm_mat(sub_dyn, type_norm='evals')
        effect_up = sub_dyn @ np.vstack(
            [np.linspace(-rec_rad, rec_rad, n_points), [rec_rad]*n_points])
        effect_down = sub_dyn @ np.vstack(
            [np.linspace(-rec_rad, rec_rad, n_points), [-rec_rad]*n_points])
        effect_right = sub_dyn @ np.vstack(
            [[rec_rad]*n_points, np.linspace(-rec_rad, rec_rad, n_points)])
        effect_left = sub_dyn @ np.vstack(
            [[-rec_rad]*n_points, np.linspace(-rec_rad, rec_rad, n_points)])
        ax.plot(effect_up[0, :], effect_up[1, :],
                alpha=alpha, color=colors[0], lw=lw)
        ax.plot(effect_down[0, :], effect_down[1, :],
                alpha=alpha, color=colors[1], lw=lw)
        ax.plot(effect_right[0, :], effect_right[1, :],
                alpha=alpha, color=colors[2], lw=lw)
        ax.plot(effect_left[0, :], effect_left[1, :],
                alpha=alpha, color=colors[3], lw=lw)
        # Up
        add_arrow(ax, [0, rec_rad], [np.mean(effect_up[0, :]), np.mean(
            effect_up[1, :])], arrowprops={'facecolor': colors[0]})
        add_arrow(ax, [0, -rec_rad], [np.mean(effect_down[0, :]),
                  np.mean(effect_down[1, :])], arrowprops={'facecolor': colors[1]})
        add_arrow(ax, [rec_rad, 0], [np.mean(effect_right[0, :]), np.mean(
            effect_right[1, :])], arrowprops={'facecolor': colors[2]})
        add_arrow(ax, [-rec_rad, 0], [np.mean(effect_left[0, :]),
                  np.mean(effect_left[1, :])], arrowprops={'facecolor': colors[3]})
...
def add_dummy_sub_legend(ax, colors, lenf, label_base='f'):
    ...
    for i, color in enumerate(colors[:lenf]):
        dummy_lines.append(
            ax.plot([], [], c=color, label='%s %s' % (label_base, str(i)))[0])
    ax.set_title('Dynamics colored by mix of colors of the dominant dynamics')
    legend = ax.legend([dummy_lines[i] for i in range(len(dummy_lines))], [
                       'f %s' % str(i) for i in range(len(colors))], loc='upper left')
    ax.legend()
...
def plot_subs_effects_2d(F, colors=[['r', 'maroon', 'darkred', 'coral'], ['forestgreen', 'limegreen', 'darkgreen', 'springgreen']], alpha=0.7, rec_rad_all=5,
                         ...
                         axs=[], fig=[]):
    if include_dyn:
        fig, axs = plt.subplots(len(F), 3, figsize=(
            35, 8*len(F)), sharey='col', sharex='col')
    else:
        if isinstance(axs, list) and len(axs) == 0:
            fig, axs = plt.subplots(len(F), 2, figsize=(
                30, 8*len(F)), sharey='col', sharex='col')

    if isinstance(colors[0], list):
        [plot_sub_effect(f_i, rec_rad_all, colors[i], alpha, axs[i, 1], n_points, params_labels={
                         'title': 'f %s effect' % str(i+1)}, lw=lw) for i, f_i in enumerate(F)]

    else:
        [plot_sub_effect(f_i, rec_rad_all, colors, alpha, axs[i, 1], n_points, params_labels={
                         'title': 'f %s effect' % str(i+1)}, lw=lw) for i, f_i in enumerate(F)]

    [plot_evals_evecs(axs[i, 0], f_i, evec_colors[i], alpha)
     for i, f_i in enumerate(F)]
    dummy_lines = []
    dummy_lines.append(axs[0, 0].plot([], [], c="black", ls='--', lw=lw)[0])
    dummy_lines.append(axs[0, 0].plot([], [], c="black", ls='-', lw=lw)[0])

    legend = axs[0, 1].legend([dummy_lines[i] for i in [0, 1]], [
                              'Original', 'after sub-dynamic transform'], loc=loc_leg)
    axs[0, 1].add_artist(legend)
    if include_dyn:
        [quiver_plot(sub_dyn=f, ax=axs[i, 2], chosen_color=evec_colors[i], type_plot='streamplot',
                     cons_color=True, xlabel='dv', ylabel='dw') for i, f in enumerate(F)]
        [axs[i, 2].set_title('f %s' % str(i+1), fontsize=18)
...
def plot_subs(F, axs=[], params_F_plot={'cmap': 'PiYG'}, include_sup=True, annot=True):
    ...
    [sns.heatmap(f_i, ax=axs[i], annot=annot, **params_F_plot)
     for i, f_i in enumerate(F)]
    [ax.set_title('f#%g' % i) for i, ax in enumerate(axs)]
    if include_sup:
        plt.suptitle('Sub-Dynamics')
...
def plot_evals_evecs(ax, sub_dyn, colors=['r', 'g', 'b', 'm'], alpha=0.7, title='evals'):
    ...
    for eval_num, eigenval in enumerate(eigenvalues):
        ...
        ax.scatter(np.real(eigenval), np.imag(eigenval),
                   alpha=alpha, color=colors, s=300)  # [eval_num])
    ax.set_xlabel('Real')
    ax.set_ylabel('Imag')
    ax.axhline(0, alpha=0.1, color='black', ls='dotted')
    ax.axvline(0, alpha=0.1, color='black', ls='dotted')
...
def plot_3d_color_scatter(latent_dyn, coefficients, ax=[], figsize=(15, 10), delta=0.4, colors=[]):
    ...
    if latent_dyn.shape[0] != 3:
        ...
        for row in range(coefficients.shape[0]):
            ...
            ax.scatter(latent_dyn[0, :]+delta*row, latent_dyn[1, :]+delta*row,
                       latent_dyn[2, :]+delta, s=coefficients_row**0.3, c=colors[row])
...
def add_arrow(ax, start, end, arrowprops={'facecolor': 'black', 'width': 1, 'alpha': 0.2}):
    arrowprops = {**{'facecolor': 'black', 'width': 1.5,
                     'alpha': 0.2, 'edgecolor': 'none'}, **arrowprops}
...
def quiver_plot(sub_dyn=[], xmin=-5, xmax=5, ymin=-5, ymax=5, ax=[], chosen_color='red',
    ...
    if ymin >= ymax:
        ...
        if not quiver_3d:
            ...
            if type_plot == 'quiver':
                h = ax.quiver(X, Y, U, V, color=chosen_color,
                              alpha=alpha, width=w)
                ...
                if cons_color:
                    ...
                    new_mat_color = np.abs(
                        new_mat  @ np.vstack([x.flatten(), y.flatten()]))
                    color_stream = new_mat_color.T @ new_mat_color
                try:
                    h = ax.streamplot(np.linspace(xmin, xmax, 100), np.linspace(ymin, ymax, 100), U.reshape(
                        X.shape), V.reshape(Y.shape), color=color_stream)  # chosen_color
                except:
                    h = ax.streamplot(np.linspace(xmin, xmax, 100), np.linspace(ymin, ymax, 100), U.reshape(
                        X.shape), V.reshape(Y.shape), color=chosen_color)  # chosen_color
            else:
                raise NameError('Wrong plot name')
        else:
            if isinstance(ax, list) and len(ax) == 0:
                fig, ax = plt.subplots(subplot_kw={'projection': '3d'})
            X, Y, Z = np.meshgrid(np.arange(xmin, xmax, inter), np.arange(
                ymin, ymax, inter), np.arange(zmin, zmax, inter))
            ...
            for xloc in np.arange(X.shape[0]):
                for yloc in np.arange(X.shape[1]):
                    for zloc in np.arange(X.shape[2]):
                        U[xloc, yloc, zloc] = new_mat[0, :] @ np.array(
                            [X[xloc, yloc, zloc], Y[xloc, yloc, zloc], Z[xloc, yloc, zloc]]).reshape((-1, 1))
                        V[xloc, yloc, zloc] = new_mat[1, :] @ np.array(
                            [X[xloc, yloc, zloc], Y[xloc, yloc, zloc], Z[xloc, yloc, zloc]]).reshape((-1, 1))
                        W[xloc, yloc, zloc] = new_mat[2, :] @ np.array(
                            [X[xloc, yloc, zloc], Y[xloc, yloc, zloc], Z[xloc, yloc, zloc]]).reshape((-1, 1))

            if type_plot == 'quiver':
                h = ax.quiver(X, Y, Z, U, V, W, color=chosen_color, alpha=alpha, lw=1.5,
                              length=0.8, normalize=True, arrow_length_ratio=0.5)  # , width = w
                ax.grid(False)
            elif type_plot == 'streamplot':
...
def visualize_dyn(dyn, ax=[], params_plot={}, turn_off_back=False, marker_size=10, include_line=False,
    ...
    """
    if not isinstance(color_sig, list) and not isinstance(color_sig, np.ndarray):
        color_sig = [color_sig]
    ...
    if len(legend) > 0:
        [ax.scatter([], [], c=colors_dyns[i], label=legend[i], s=10)
    ...
    if color_by_dominant and (coefficients.shape[1] == dyn.shape[1]-1 or coefficients.shape[1] == dyn.shape[1]):
        if color_mix:
            if len(colors) == 0 or not np.shape(colors)[0] == 3:
                raise ValueError('colors mat should have 3 rows')
            else:

                color_sig = ((np.array(colors)[:, :coefficients.shape[0]] @ np.abs(
                    coefficients)) / np.max(np.abs(coefficients).sum(0).reshape((1, -1)))).T
            ...
            if len(color_sig.flatten()) < dyn.shape[1]:
                dyn = dyn[:, :len(color_sig.flatten())]
            if color_for_0:
    ...
    if dyn.shape[0] > 2:
        if len(colors) == 0:
            ...
            h = ax.scatter(dyn[0, :], dyn[1, :], dyn[2, :],
                           marker=marker, s=marker_size, c=color_sig, alpha=alpha)
    else:
        dyn = np.array(dyn)
        ...
        if len(colors) == 0:
            ...
            h = ax.scatter(dyn[0, :], dyn[1, :],  marker=marker,
                           s=marker_size, c=color_sig, alpha=alpha)
    ...
    if len(text_points) > 0:
        ...
        if dyn.shape[0] == 3:
            [ax.text(dyn[0, t]+delta_text, dyn[1, t]+delta_text, dyn[2, t]+delta_text, '%s = %s' %
                     (add_text, str(t)),  fontsize=fontsize_times, fontweight='bold') for t in text_points]
        else:
            [ax.text(dyn[0, t]+delta_text, dyn[1, t]+delta_text, '%s = %s' % (add_text,
                     str(t)),  fontsize=fontsize_times, fontweight='bold') for t in text_points]
...
def str2bool(str_to_change):
    ...
    if isinstance(str_to_change, str):
        str_to_change = (str_to_change.lower() == 'true') or (
            str_to_change.lower() == 'yes')
    return str_to_change


def norm_over_time(coefficients, type_norm='normal'):
    if type_norm == 'normal':
        coefficients_norm = (coefficients - np.mean(coefficients,
                             1).reshape((-1, 1)))/np.std(coefficients, 1).reshape((-1, 1))
...
def norm_coeffs(coefficients, type_norm, same_width=True, width_des=0.7, factor_power=0.9, min_width=0.01):
    ...
    """
    if type_norm == 'norm':
        coefficients_n = norm_over_time(
            np.abs(coefficients), type_norm='normal')
        coefficients_n = coefficients_n - \
            np.min(coefficients_n, 1).reshape((-1, 1))

    elif type_norm == 'sum_abs':
        coefficients[np.abs(coefficients) < min_width] = min_width
        coefficients_n = np.abs(coefficients) / \
            np.sum(np.abs(coefficients), 1).reshape((-1, 1))
    elif type_norm == 'abs':
        coefficients[np.abs(coefficients) < min_width] = min_width
        coefficients_n = np.abs(coefficients)
    ...
    if same_width:
        coefficients_n = width_des*(np.abs(coefficients_n)**factor_power) / \
            np.sum(np.abs(coefficients_n)**factor_power, axis=0)
    else:
        coefficients_n = np.abs(coefficients_n) / \
            np.sum(np.abs(coefficients_n), axis=0)
    coefficients_n[coefficients_n < min_width] = min_width
...
def movmfunc(func, mat, window=3, direction=0):
    ...
    if direction == 0:
        mat_wrap = np.vstack([np.nan*np.ones((addition, np.shape(mat)[1])),
                             mat, np.nan*np.ones((addition, np.shape(mat)[1]))])
        movefunc_res = np.vstack([func(mat_wrap[i-addition:i+addition, :], axis=direction)
                                 for i in range(addition, np.shape(mat_wrap)[0]-addition)])
    elif direction == 1:
        mat_wrap = np.hstack([np.nan*np.ones((np.shape(mat)[0], addition)),
                             mat, np.nan*np.ones((np.shape(mat)[0], addition))])
        movefunc_res = np.vstack([func(mat_wrap[:, i-addition:i+addition], axis=direction)
                                 for i in range(addition, np.shape(mat_wrap)[1]-addition)]).T
    return movefunc_res
...
def create_reco(latent_dyn, coefficients, F, type_find='median', min_far=10, smooth_coeffs=False,
    ...
    if smooth_coeffs:
        coefficients = movmfunc(
            np.nanmedian, coefficients, window=smoothing_params['wind'], direction=1)
...
def add_labels(ax, xlabel='X', ylabel='Y', zlabel='Z', title='', xlim=None, ylim=None, zlim=None, xticklabels=np.array([None]),
    ...
    Inputs:
        ...
        etc.
    """
    if xlabel != '' and xlabel != None:
        ax.set_xlabel(xlabel, **xlabel_params)
    if ylabel != '' and ylabel != None:
        ax.set_ylabel(ylabel, **ylabel_params)
    if zlabel != '' and zlabel != None:
        ax.set_zlabel(zlabel, **ylabel_params)
...
def check_save_name(save_name, invalid_signs='!@#$%^&*.,:;', addi_path=[], sep=sep):
    ...
    for invalid_sign in invalid_signs:
        save_name = save_name.replace(invalid_sign, '_')
    if len(addi_path) == 0:
        return save_name
    else:
...
def load_vars(folders_names,  save_name, sep=sep, ending='.pkl', full_name=False):
    ...
    if full_name:
        ...
        if not save_name.endswith(ending):
            save_name = '%s%s' % (save_name, ending)
...
def create_colors(len_colors, perm=[0, 1, 2]):
    ...
    """
    colors = np.vstack([np.linspace(0, 1, len_colors), (1-np.linspace(0,
                       1, len_colors))**2, 1-np.linspace(0, 1, len_colors)])
    colors = colors[perm, :]
    return colors
...
def find_perpendicular(d1, d2, perp_length=1, prev_v=[], next_v=[], ref_point=[], choose_meth='intersection', initial_point='mid',
    ...
    if initial_point == 'mid':
        ...
        d1_perp = perp_begin
    elif initial_point == 'end':
        d1_perp = d2
    elif initial_point == 'start':
        d1_perp = d1
    else:
        raise NameError('Unknown intial point')

    # If perpendicular direction is according to 'intersection' elimination
    if choose_meth == 'intersection':
        if len(prev_v) > 0:
            intersected_curve1 = prev_v
        else:
            intersected_curve1 = d1
        if len(next_v) > 0:
            intersected_curve2 = next_v
        else:
            intersected_curve2 = d2

    # If a horizontal line
    if d2[0] == d1[0]:
        d2_perp = np.array([d1_perp[0]+perp_length, d1_perp[1]])
    # If a vertical line
    elif d2[1] == d1[1]:
        d2_perp = np.array([d1_perp[0], d1_perp[1]+perp_length])
    else:
        ...
        if choose_meth == 'smooth' or choose_meth == 'intersection':
            if len(ref_point) == 0:
                ...
                smooth_val = np.array(ref_point)

        # if by convexity
        if choose_meth == 'prev':
            if len(prev_v) > 0 and len(next_v) > 0:                     # both sides are provided
                prev_mid_or = (np.array(prev_v) + np.array(next_v))/2
            # only the previous side is provided
            elif len(prev_v) > 0 and len(next_v) == 0:
                prev_mid_or = (np.array(prev_v) + np.array(d2))/2
            # only the next side is provided
            elif len(next_v) > 0 and len(prev_v) == 0:
                prev_mid_or = (np.array(d1) + np.array(next_v))/2
            else:
                raise ValueError(
                    'prev or next should be defined (to detect convexity)!')
        ...
        if len(prev_mid) > 0:
            ...
            # here ref point is a point of a different dynamics layer from which we want to take distance
            if len(ref_point) > 0 and layer_num > 0:
                dist1 = np.sum((smooth_val - d2_perp1)**2)
                dist2 = np.sum((smooth_val - d2_perp2)**2)
                max_opt = np.argmax([dist1, dist2])

            elif choose_meth == 'intersection':
                dist1 = np.min(min_dist(prev_mid, d2_perp1,
                               intersected_curve1, intersected_curve2))
                dist2 = np.min(min_dist(prev_mid, d2_perp2,
                               intersected_curve1, intersected_curve2))
                max_opt = np.argmax([dist1, dist2])
...
def find_lows_high(coeff_row, latent_dyn,   choose_meth='intersection', factor_power=0.9, initial_point='start',
    ...
    for t_num in range(0, latent_dyn.shape[1]-2):
        ...
        # and ref_shape_all.shape[1] >1
        if len(ref_shape_all) > 0 and ref_shape_all.shape[0] > t_num and layer_num > 0:
        ...
        if return_unchose:
            d1_perp, d2_perp, d2_perp_unchosen = find_perpendicular(d1_coeff, d2_coeff, c_len**factor_power, prev_v=prev_v, next_v=next_v, ref_point=ref_point,
                                                                    choose_meth=choose_meth, initial_point=initial_point, direction_initial=direction_initial, return_unchose=return_unchose, layer_num=layer_num)  # c_len
        else:
            d1_perp, d2_perp = find_perpendicular(d1_coeff, d2_coeff, c_len**factor_power, prev_v=prev_v, next_v=next_v, ref_point=ref_point, choose_meth=choose_meth,
                                                  initial_point=initial_point, direction_initial=direction_initial, return_unchose=return_unchose, layer_num=layer_num)  # c_len
        # Add results to results lists
        x_lows_y_lows.append([d1_perp[0], d1_perp[1]])
        x_highs_y_highs.append([d2_perp[0], d2_perp[1]])
        if return_unchose:
            unchosen_highs.append([d2_perp_unchosen[0], d2_perp_unchosen[1]])
    # return
    if return_unchose:
...
def plot_multi_colors(store_dict, min_time_plot=0, max_time_plot=-100,  colors=['green', 'red', 'blue'], ax=[],
    ...
    for key_counter, (key, set_plot) in enumerate(store_dict.items()):
        if key_counter == 0:
            ...
            low_ref = [np.array(x_highs_y_highs)[min_time_plot, 0],   np.array(
                x_highs_y_highs)[min_time_plot, 1]]
            high_ref = [np.array(x_highs_y_highs)[max_time_plot, 0], np.array(
                x_highs_y_highs)[max_time_plot, 1]]
        if len(coefficients_n) > 0:
            # Define the length of the last perp.
            c_len = (coefficients_n[key, max_time_plot-1] +
                     coefficients_n[key, max_time_plot])/2
            ...
            d1_p, d2_p = find_perpendicular([np.array(x_lows_y_lows)[max_time_plot-2, 0], np.array(x_lows_y_lows)[max_time_plot-2, 1]],
                                            [np.array(x_lows_y_lows)[
                                                max_time_plot-1, 0], np.array(x_lows_y_lows)[max_time_plot-1, 1]],
                                            perp_length=c_len**factor_power,
                                            ref_point=high_ref,
                                            choose_meth='intersection', initial_point='end')
            # Define the length of the first perp.
            c_len_start = (
                coefficients_n[key, min_time_plot-1] + coefficients_n[key, min_time_plot])/2
            # Create perp. in the first point
            d1_p_start = [np.array(x_highs_y_highs)[min_time_plot, 0], np.array(
                x_highs_y_highs)[min_time_plot, 1]]

            d2_p_start = [np.array(x_highs_y_highs)[
                min_time_plot+1, 0], np.array(x_highs_y_highs)[min_time_plot+1, 1]]

            x_lows_y_lows = store_dict[key][0]
            x_highs_y_highs = store_dict[key][1]

            stack_x = np.hstack([np.array(x_lows_y_lows)[min_time_plot:max_time_plot, 0].flatten(), np.array(
                [d2_p[0]]), np.array(x_highs_y_highs)[max_time_plot-1:min_time_plot+1:-1, 0].flatten(), np.array([d2_p_start[0]])])
            stack_y = np.hstack([np.array(x_lows_y_lows)[min_time_plot:max_time_plot, 1].flatten(), np.array(
                [d2_p[1]]), np.array(x_highs_y_highs)[max_time_plot-1:min_time_plot+1:-1, 1].flatten(), np.array([d2_p_start[1]])])

        else:
            stack_x = np.hstack([np.array(x_lows_y_lows)[min_time_plot:max_time_plot, 0].flatten(
            ), np.array(x_highs_y_highs)[max_time_plot:min_time_plot:, 0].flatten()])
            stack_y = np.hstack([np.array(x_lows_y_lows)[min_time_plot:max_time_plot, 1].flatten(
            ), np.array(x_highs_y_highs)[max_time_plot:min_time_plot:, 1].flatten()])
        stack_x_smooth = stack_x
        stack_y_smooth = stack_y
        if key_counter != 0:
            ax.fill(stack_x_smooth, stack_y_smooth, alpha=0.3,
                    facecolor=colors[key_counter], edgecolor=None, zorder=1, snap=True)
        else:
            ax.fill(stack_x_smooth, stack_y_smooth, alpha=alpha,
                    facecolor=colors[key_counter], edgecolor=None, zorder=1, snap=True)

    if to_scatter or (to_scatter_only_one and key == np.max(list(store_dict.keys()))):

        ax.scatter(np.array(x_lows_y_lows)[min_time_plot:max_time_plot, 0].flatten(), np.array(
            x_lows_y_lows)[min_time_plot:max_time_plot, 1].flatten(), c='black', alpha=alpha, s=45)
...
def calculate_high_for_all(coefficients, choose_meth='both', same_width=True, factor_power=0.9, width_des=0.7,
    ...
    """
    if len(latent_dyn) == 0:
        raise ValueError('Empty latent dyn was provided')

    # Coeffs normalization
    coefficients_n = norm_coeffs(coefficients, type_norm, same_width=same_width,
                                 width_des=width_des, factor_power=factor_power, min_width=min_width)
    ...
    for row in range(coefficients_n.shape[0]):
        ...
        if return_unchose:
            ...
            store_dict[row] = [x_lows_y_lows,
                               x_highs_y_highs, x_highs_y_highs2]
        else:
        ...
        if initial_point == 'mid':
            ...
            dyn_use = np.hstack(
                [latent_dyn[:, :2], dyn_use, latent_dyn[:, -2:]])
        else:
            dyn_use = np.array(x_highs_y_highs).T
...
def plot_3d_dyn_basis(F, coefficients, projection=[0, -1], ax=[],  fig=[], time_emph=[], n_times=5,
    ...
    if len(colors) == 0:
        if color_by_dom:
            ...
            colors_base = np.linspace(
                0, 1, coefficients.shape[1]).reshape((-1, 1))
            colors = np.hstack([colors_base, 1-colors_base, colors_base**2])

    if isinstance(ax, list) and len(ax) == 0:
        if len(F) == 3:
            fig, ax = plt.subplots(
                subplot_kw={'projection': '3d'}, figsize=(10, 10))
        elif len(F) == 2:
            fig, ax = plt.subplots(figsize=(10, 10))
        else:
            raise ValueError('Invalid dim for F')
    if len(time_emph) == 0:
        time_emph = np.linspace(
            0, coefficients.shape[1]-2, n_times+1)[1:].astype(int)

    if include_dynamics:

        if len(latent_dyn) == 0:
            raise ValueError(
                'You should provide latent dyn as input if "include dynamics" it True')
        if len(F[0]) == 3:
            fig_dyn, ax_dyn = plt.subplots(
                figsize=(15, 15), subplot_kw={'projection': '3d'})
            if new_colors:

                ax_dyn.scatter(latent_dyn[0, :len(colors_base)], latent_dyn[1, :len(
                    colors_base)], latent_dyn[2, :len(colors_base)], color=colors, alpha=0.3)
                ax_dyn.scatter(latent_dyn[0, time_emph], latent_dyn[1,
                               time_emph], latent_dyn[2, time_emph], c='black', s=300)
            else:
                c_sig = np.arange(latent_dyn.shape[1])
                ax_dyn.scatter(latent_dyn[0, :], latent_dyn[1, :],
                               latent_dyn[2, :], c=c_sig, alpha=0.3, cmap='viridis', s=100)
                ax_dyn.scatter(latent_dyn[0, time_emph], latent_dyn[1, time_emph],
                               latent_dyn[2, time_emph], c=c_sig[time_emph], s=300, cmap='viridis')
            [ax_dyn.text(latent_dyn[0, t] + delta_text, latent_dyn[1, t]+delta_text_y, latent_dyn[2, t] +
                         delta_text_z, 't = %s' % str(t), fontsize=fontsize_times, fontweight='bold') for t in time_emph]
            ax_dyn.set_axis_off()
            ...
            if new_colors:
                ...
                ax_dyn.scatter(latent_dyn[0, :len(colors_base)], latent_dyn[1, :len(
                    colors_base)], color=colors, alpha=0.3, s=50)
                ax_dyn.scatter(
                    latent_dyn[0, time_emph], latent_dyn[1, time_emph], c='black', s=200)
            else:
                c_sig = np.arange(latent_dyn.shape[1])
                ax_dyn.scatter(
                    latent_dyn[0, :], latent_dyn[1, :], c=c_sig, alpha=0.3, cmap='viridis', s=100)
                ax_dyn.scatter(latent_dyn[0, time_emph], latent_dyn[1,
                               time_emph], c=c_sig[time_emph], s=300, cmap='viridis')
            [ax_dyn.text(latent_dyn[0, t] + delta_text, latent_dyn[1, t]+delta_text_y, 't = %s' %
                         str(t), fontsize=fontsize_times, fontweight='bold') for t in time_emph]
            remove_edges(ax_dyn)
    ...
    if len(F[0]) == 3:
        if quiver_3d:
            if type_plot == 'streamplot':
                type_plot = 'quiver'
                print(
                    'If quiver_3d then type_plot need to be quiver (currently is streamplot)')
            if include_quiver:
                if isinstance(ax_qui, list) and len(ax_qui) == 0:
                    fig_qui, ax_qui = plt.subplots(1, len(time_emph), figsize=(
                        7*len(time_emph), 5), subplot_kw={'projection': '3d'})
            if isinstance(ax_base, list) and len(ax_base) == 0:
                if base_narrow:
                    fig_base, ax_base = plt.subplots(len(F), 1, figsize=(
                        5, 7*len(F)), subplot_kw={'projection': '3d'})
                else:
                    fig_base, ax_base = plt.subplots(1, len(F), figsize=(
                        7*len(F), 5), subplot_kw={'projection': '3d'})
        else:
            ...
            if include_quiver:
                if isinstance(ax_qui, list) and len(ax_qui) == 0:
                    fig_qui, ax_qui = plt.subplots(
                        1, len(time_emph), figsize=(7*len(time_emph), 5))
            if isinstance(ax_base, list) and len(ax_base) == 0:
                if base_narrow:
                    fig_base, ax_base = plt.subplots(
                        len(F), 1, figsize=(5, 7*len(F)))
                else:
                    fig_base, ax_base = plt.subplots(
                        1, len(F), figsize=(7*len(F), 5))
    ...
    if len(F[0]) == 3:
        ...
        if new_colors:
            ...
            ax.scatter(coefficients[0, :], coefficients[1, :],
                       coefficients[2, :], c=colors, alpha=alpha0, s=s_all)
            ...
            ax.scatter(coefficients[0, :], coefficients[1, :],
                       coefficients[2, :], c=colors_base, alpha=alpha0, s=s_all)
            ax.scatter(coefficients[0, time_emph], coefficients[1, time_emph],
                       coefficients[2, time_emph], c='black', s=s, alpha=np.min([alpha0*2, 1]))
            if include_quiver:
        ...
        if to_title and include_quiver:
            if loc_title == 'title':
                [ax_qui[i].set_title('t = ' + str(time_point), fontsize=fontsize_times*3,
                                     fontweight='bold') for i, time_point in enumerate(time_emph)]
            else:
                [ax_qui[i].set_ylabel('t = ' + str(time_point), fontsize=fontsize_times,
                                      fontweight='bold') for i, time_point in enumerate(time_emph)]

        [ax.text(coefficients[0, time_point]+delta_text, coefficients[1, time_point]+delta_text_y, coefficients[2, time_point] +
                 delta_text_z, 't = ' + str(time_point), fontsize=fontsize_times, fontweight='bold') for time_point in time_emph_text]
        ax.set_xlabel('f1')
        ax.set_ylabel('f2')
        ax.set_zlabel('f3')
        ...
        if new_colors:
            ax.scatter(coefficients[0, :], coefficients[1,
                       :], c=colors, alpha=alpha0, s=s_all)
            ax.scatter(
                coefficients[0, time_emph], coefficients[1, time_emph], c=colors[time_emph], s=s)
            if include_quiver:
                [plot_reco_dyn(coefficients, F, time_point, type_plot=type_plot, range_p=range_p, color=colors[time_point],
                               w=w, ax=ax_qui[i], cons_color=cons_color) for i, time_point in enumerate(time_emph)]
        else:
            colors_base = np.arange(coefficients.shape[1])
            ax.scatter(coefficients[0, :], coefficients[1, :],
                       c=colors_base, alpha=alpha0, s=s_all)
            ax.scatter(coefficients[0, time_emph], coefficients[1,
                       time_emph], c=colors_base[time_emph], s=s)
            if include_quiver:
                [plot_reco_dyn(coefficients, F, time_point, type_plot=type_plot, range_p=range_p, color=cmap(
                    time_point/colors.shape[0]), w=w, ax=ax_qui[i], cons_color=cons_color) for i, time_point in enumerate(time_emph)]
        if latent_dyn.shape[0] == 3:
            [ax.text(coefficients[0, time_point]+delta_text, coefficients[1, time_point]+delta_text_y, coefficients[2, time_point] +
                     delta_text_z, 't = ' + str(time_point), fontsize=fontsize_times, fontweight='bold') for time_point in time_emph_text]

        else:
            [ax.text(coefficients[0, time_point]+delta_text, coefficients[1, time_point]+delta_text_y, 't = ' +
                     str(time_point), fontsize=fontsize_times, fontweight='bold') for time_point in time_emph_text]
        ...
        if to_title and include_quiver:
            if loc_title == 'title':
                [ax_qui[i].set_title('t = ' + str(time_point), fontsize=30)
                 for i, time_point in enumerate(time_emph)]
            else:
                [ax_qui[i].set_ylabel('t = ' + str(time_point), fontsize=30)
                 for i, time_point in enumerate(time_emph)]
    if to_remove_edge:
        if include_quiver:
            [remove_edges(ax_spec) for ax_spec in ax_qui]
        [remove_edges(ax_spec) for ax_spec in ax_base]
        ax.set_xticks([])
        ax.set_yticks([])
        if quiver_3d:
            ax.set_zticks([])
    [quiver_plot(f, -range_p, range_p, -range_p, range_p, ax=ax_base[f_num], chosen_color='black', w=w,
                 type_plot=type_plot, cons_color=cons_color, quiver_3d=quiver_3d) for f_num, f in enumerate(F)]
    [ax_base_spec.set_title('f %s' % str(i), fontsize=16)
     for i, ax_base_spec in enumerate(ax_base)]
    ...
    if include_bar:
        if base_narrow:
            fig_all_all, ax_all_all = plt.subplots(
                len(time_emph), 1, figsize=(6, len(time_emph)*7))
        else:
            ax_all_all = []
        ...
        if isinstance(axs_basis_colored, list) and len(axs_basis_colored) == 0:
            if base_narrow:
                if quiver_3d:
                    fig_basis_colored, axs_basis_colored = plt.subplots(
                        len(F), 1, figsize=(5, 6*len(F)), subplot_kw={'projection': '3d'})
                else:
                    fig_basis_colored, axs_basis_colored = plt.subplots(
                        len(F), 1, figsize=(5, 6*len(F)))

            else:
                if quiver_3d:
                    fig_basis_colored, axs_basis_colored = plt.subplots(
                        1, len(F), figsize=(6*len(F), 5), subplot_kw={'projection': '3d'})
                else:
                    fig_basis_colored, axs_basis_colored = plt.subplots(
                        1, len(F), figsize=(6*len(F), 5))
        [quiver_plot(f, -range_p, range_p, -range_p, range_p, ax=axs_basis_colored[f_num], alpha=0.7, chosen_color=colors_dyns[f_num],
                     w=w, type_plot=type_plot, cons_color=cons_color, quiver_3d=quiver_3d) for f_num, f in enumerate(F)]
        [remove_edges(ax_spec) for ax_spec in axs_basis_colored]
        if quiver_3d:
            [ax.set_zticks([]) for ax in axs_basis_colored]
...
def plot_reco_dyn(coefficients, F, time_point, type_plot='quiver', range_p=10, color='black',
    ...
    if len(F) == 3:
        merge_dyn_at_t_break = coefficients[0, time_point] * F[0] + \
            coefficients[1, time_point] * F[1] + \
            coefficients[2, time_point] * F[2]
        ...
        merge_dyn_at_t_break = coefficients[0, time_point] * \
            F[0]+coefficients[1, time_point] * F[1]
...
def plot_c_space(coefficients, latent_dyn=[], axs=[], fig=[], xlim=[-50, 50], ylim=[-50, 50], add_midline=True, d3=True, cmap='winter', color_sig=[],
    ...
    if len(times_plot) > 0 and isinstance(cmap_f, list):
        cmap_f = plt.cm.get_cmap(cmap)
    if len(color_sig) == 0:
        color_sig = latent_dyn[0, :-1]
    if isinstance(axs, list) and len(axs) == 0:
        ...
        if coefficients.shape[0] == 3:
            ...
            d3 = True
            h = axs.scatter(
                coefficients[0, :], coefficients[1, :], coefficients[2, :], c=color_sig, cmap=cmap)
            if len(times_plot) > 0:
                axs.scatter(coefficients[0, times_plot], coefficients[1, times_plot], coefficients[2, times_plot], c=cmap_f(
                    color_sig[times_plot]/np.max(color_sig)), s=500)
        elif coefficients.shape[0] == 2:
            if d3:
                fig, axs = plt.subplots(
                    figsize=(15, 15), subplot_kw={'projection': '3d'})
                h = axs.scatter(coefficients[0, :], coefficients[1, :], np.arange(
                    coefficients.shape[1]), c=color_sig, cmap=cmap)
                if len(times_plot) > 0:
                    zax = np.arange(coefficients.shape[1])
                    axs.scatter(coefficients[0, times_plot], coefficients[1, times_plot], zax[times_plot], c=cmap_f(
                        color_sig[times_plot]/np.max(color_sig)), s=500)
            else:
                fig, axs = plt.subplots(figsize=(15, 15))
                h = axs.scatter(
                    coefficients[0, :], coefficients[1, :], c=color_sig, cmap=cmap)
                if len(times_plot) > 0:
                    axs.scatter(coefficients[0, times_plot], coefficients[1, times_plot],  c=cmap_f(
                        color_sig[times_plot]/np.max(color_sig)), s=500)
        else:
    ...
    if add_midline:
        if d3:
            ...
            axs.plot([0, 0], [np.min(coefficients[1, :]), np.max(coefficients[1, :])], [
                     0, 0], color='black', ls='--', alpha=0.3)
            axs.plot([np.min(coefficients[0, :]), np.nanmax(coefficients[0, :])], [
                     0, 0], [0, 0], color='black', ls='--', alpha=0.3)
            axs.plot([0, 0], [0, 0], [0, 1.3*coefficients.shape[1]],
